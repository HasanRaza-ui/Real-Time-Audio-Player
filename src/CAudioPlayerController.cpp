...
//		m_pFilter=NULL;
	...
	try{
		...
		float block_Duration = 0.125; //Block Duration in seconds

		long framesPerBlock = (m_pSFile -> getSampleRate()) * block_Duration;

		int sbufsize = (m_pSFile -> getNumChannels()) * framesPerBlock;
		...
		do {
			...
			if(key){
				if(!m_pFilter){
					readsize = m_pSFile -> read(sbufBlock, sbufsize);
					m_audioStream.play(sbufBlock, readsize / m_pSFile -> getNumChannels());
					m_ui.visualizeAmplitude(sbufBlock, sbufsize);
				}else{
					readsize = m_pSFile -> read(sbufBlock, sbufsize);
					m_pFilter ->filter(sbufBlock, sbufBlock_f, framesPerBlock);
					m_audioStream.play(sbufBlock_f, readsize / m_pSFile -> getNumChannels());
					m_ui.visualizeAmplitude(sbufBlock_f, sbufsize);
				}
			}
